...
class ServiceManager:
    ...
    def start_service(self, service_id, port=None):
        ...
        try:
            ...
            # Prepare the command to run the service - use workspace Python if available
            workspace_python = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'venv', 'bin', 'python3')
            python_executable = workspace_python if os.path.exists(workspace_python) else sys.executable
            cmd = [python_executable, os.path.basename(script_path)]
